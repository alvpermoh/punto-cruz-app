...
def matriz_a_imagen(matriz, filename=None, offset_x=0, offset_y=0):
    ...
    #plt.tight_layout()
    filename = os.path.join("static/bloques", filename)
    if filename:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close(fig)  # Evita que se acumulen figuras en memoria
    
    

    return filename

def leyenda_a_imagen(legend, conteo_simbolos, filename="leyenda.png"):
    n = len(legend)
...
def divide_matrix(matrix, block_size=30):
    ...
    for y in range(0, h, block_size):
        for x in range(0, w, block_size):
            block = [row[x:x + block_size] for row in matrix[y:y + block_size]]
...
def convert_image_to_dmc(image_path, output_path, palette_name, max_side_length):
    ...
    symbol_image_path=apply_function_to_blocks(output_img2, matriz_a_imagen, block_size=50)
    print("Hecho 2")
    matriz_a_imagen(output_img2, "resultado.png")

    # Crear leyenda
    create_legend(legend, output_path)
    leyenda_path=leyenda_a_imagen(legend=legend,conteo_simbolos=conteo_simbolos, filename="leyenda.png")

    cell_size = 10  # o el tamaño que quieras
    grid = [list(legend.values())]  # Ejemplo simple, una fila con los símbolos usados

    # Generar PDF
    generate_pdf(output_path, grid, legend, cell_size, converted_image_path,symbol_image_path,leyenda_path)
    print("Hecho 3")



def generate_pdf(output_path, grid, legend, cell_size, image_path, image_paths,leyenda_path):
    ...
    x = (page_width - width) / 2
    y = (page_height - height) / 2 
    c.drawImage(leyenda_path, x, y, width, height)
    c.save()
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        _, input_path, output_path, palette = sys.argv
        convert_image_to_dmc(input_path, output_path, palette.lower(),165)
